...
class BDF2():
    def __init__(self, max_newton_iter=50, newton_tol=1e-6):
    ...
    def update_state(self, q_sol, t_vec, step_size, rhs):
        ...
        return q_old, t_vec[-1]+step_size
...
class ImplicitEuler():
    ...
    def update_state(self, t_vec, q_sol, rhs, step_size):
        ...
        self.state_len =  q_old.shape[0]

        time = t_vec[-1]

        self.J = self.compute_jacobian(time,q_old,self.state_len,rhs)

        LHS = 1/step_size*np.eye(self.state_len) - self.J

        while newton_error > self.newton_tol and \
                iterations < self.max_newton_iter:
            RHS = -(1/step_size*(q_new - q_old) - rhs(time,q_new))
