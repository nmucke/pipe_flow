function [unew, nsolves, res] = RKstep_esdirk(un,tn,dt,options)
% one time step with explicit first stage diagonally implicit Runge-Kutta
% (ESDIRK) method starting from un to unew starting from tn with time step dt
% nsolves is number of solves of (M-1)x(M-1) system, with M the dimension of un
...
% initialize right-hand side of the first stage, which is explicit
F_RK(1,:) = getRHS(un,tn,options);
