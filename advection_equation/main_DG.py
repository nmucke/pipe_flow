...
import pdb
import advection_global as pipe_flow
...
for xl in [500]:
    ...
    FinalTime = 1#0.142857142857143

    mu1 = xmax/2
    sigma = 20
    q1init =  1 + 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-0.5 * np.power((DG_model_pipe.x - mu1) / sigma, 2)) + rho0
    ...
    q2init = np.ones((N+1,K))*q1init#initInflow*np.ones((N+1,K))*q1init
    #q2init = 1 +  1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-0.5 * np.power((DG_model_pipe.x - mu1) / sigma, 2))
    #q2init = q2init * q1init

    solq1,solq2, time = DG_model_pipe.solve(q1init,q2init, FinalTime=FinalTime,implicit=False,stepsize=2e0,
