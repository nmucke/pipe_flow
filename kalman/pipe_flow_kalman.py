...
class Pipe1D(DG.DG_1D):
    ...
    def PipeRHS1D(self,time,q):
        ...
        q1 = q[0:(self.K*self.Np)]
        q2 = q[(self.K*self.Np):(2*self.K*self.Np)]
        ...
        out = np.concatenate((rhsq1,rhsq2,q[-2],q[-1]),axis=0)
        return out
    ...
    def ExplicitIntegration(self,q1,q2,FinalTime):
        ...
        mindeltax = self.x[1,0]-self.x[0,0]
        ...
        while time < FinalTime:
            ...
            rhsq1,rhsq2 = rhs[0:int(self.m)],rhs[-int(m):]
            ...
            rhsq1,rhsq2 = rhs[0:int(self.m)],rhs[-int(self.m):]
            rhsq1, rhsq2 = np.reshape(rhsq1, (self.Np, self.K), 'F'),np.reshape(rhsq2, (self.Np, self.K), 'F')
    ...
    def solve_kalman(self, q1,q2,uncertain_pars = [], FinalTime=100,implicit=False,stepsize=1e-5,tl=0,
              leak_type='mass',pamb=1e5,mu=1.,initInflow=2,initOutPres=5e5):
        ...
        self.num_uncertain = len(uncertain_pars)
        self.m += self.num_uncertain
        self.xl = uncertain_pars[0]
        self.Cv = uncertain_pars[1]
