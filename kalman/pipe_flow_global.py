...
class Pipe1D(DG.DG_1D):
    ...
    def SolveAdjoint(self, q1, q2, obs,tVec):
        ...
        LHS = -np.eye(self.m) - self.adjStepSize*np.transpose(self.ComputeJacobian(self.time,q[-2]))
        ...
        while self.time > 0:
            ...
            LHS = -np.eye(self.m) - self.adjStepSize*np.transpose(self.ComputeJacobian(self.time,q[-(time_idx+1)]))
